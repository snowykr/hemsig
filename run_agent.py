...
class AIAgent:
    ...
    def __init__(
        ...
        self.workflow_state: Optional[ReviewResponseWorkflowState] = None
        self._workflow_state_store = ReviewResponseWorkflowStateStore()
    ...
    def reset_session_state(self):
        ...
        """
        # Runtime-only workflow context must not leak across fresh sessions.
    ...
    def activate_workflow(self, activation: WorkflowActivation | Dict[str, Any] | None) -> None:
        """Attach runtime-only workflow metadata to this agent session."""

        if activation is None:
            return
        if isinstance(activation, WorkflowActivation):
    ...
    def _active_workflow_id(self) -> Optional[str]:
        ...
        return workflow_id if workflow_id in STATE_WORKFLOW_IDS else None

    def _ensure_workflow_state_loaded(self, *, save_if_new: bool = False) -> None:
        """Load persisted targeted workflow state without touching messages."""

        workflow_id = self._active_workflow_id()
        if not workflow_id:
            self.workflow_state = None
            return
        store = getattr(self, "_workflow_state_store", None)
        if store is None:
            store = ReviewResponseWorkflowStateStore()
            self._workflow_state_store = store
        session_id = getattr(self, "session_id", "") or "unknown"
        state = store.load(session_id, workflow_id=workflow_id)
        self.workflow_state = state
        reconciled = reconcile_workflow_background_handles(state)
        if save_if_new:
            store.save(state)
        elif reconciled:
    ...
    def _persist_workflow_state(self) -> None:
        """Persist targeted workflow state when one is active."""

        if not self._active_workflow_id() or self.workflow_state is None:
            return
        store = getattr(self, "_workflow_state_store", None)
        if store is None:
            store = ReviewResponseWorkflowStateStore()
            self._workflow_state_store = store
        try:
            store.save(self.workflow_state)
        except Exception as exc:
            logger.warning(
                "Workflow state save failed for session %s: %s",
                getattr(self, "session_id", ""),
                exc,
            )

    def next_review_response_workflow_action(
        self,
        inputs: Any | None = None,
    ) -> Optional[Any]:
        """Return the next deterministic action for github-pr-review-response.

        The action is advisory orchestration data: callers must still execute any
        terminal/delegation work through the normal tool path so approvals,
        logging, budgets, and permissions remain authoritative.
        """

        if self._active_workflow_id() != "review_response":
            return None
        if self.workflow_state is None:
            self._ensure_workflow_state_loaded(save_if_new=bool(getattr(self, "session_id", "")))
        if self.workflow_state is None:
            return None
        runner = getattr(self, "_workflow_phase_runner", None)
        if runner is None:
            from importlib import import_module

            runner_cls = import_module(
                "agent.workflows.review_response_phase_runner"
            ).ReviewResponsePhaseRunner
            runner = runner_cls(store=getattr(self, "_workflow_state_store", None))
            self._workflow_phase_runner = runner
        return runner.next_action(self.workflow_state, inputs)

    def execute_review_response_workflow_action(
        self,
        action: Any,
        messages: list[dict[str, Any]],
        effective_task_id: str,
    ) -> bool:
        """Execute a workflow-owned runner action through the normal tool path.

        The phase runner is intentionally advisory.  When an action carries
        terminal arguments, this helper converts them into a normal tool-call
        shape and routes it through ``_execute_tool_calls`` so the same workflow
        policy, plugin pre-tool blocks, approval prompts, logging callbacks,
        checkpoint hooks, interrupt checks, result recording, and turn-budget
        handling remain authoritative.  It returns ``True`` when the action was
        handled (including blocked/skipped cases) and ``False`` when the action
        has no executable tool payload.
        """

        if self._active_workflow_id() != "review_response":
            return False

        terminal_args = getattr(action, "terminal_args", None)
        if not terminal_args:
            return False

        function_name = "terminal"
        tool_call_id = f"workflow_{uuid.uuid4().hex[:12]}"
        ...
        assistant_tool_call = {
            ...
            "function": {
                "name": function_name,
                "arguments": json.dumps(terminal_args, ensure_ascii=False),
            },
        }
        messages.append({
            "role": "assistant",
        ...
        def _append_synthetic_tool_result(content: str) -> None:
            messages.append({
                "role": "tool",
                "content": content,
        ...
        if self._interrupt_requested:
            self._vprint(
                f"{self.log_prefix}⚡ Interrupt: skipping workflow-owned {function_name} tool call",
                force=True,
            )
            _append_synthetic_tool_result(
                f"[Tool execution cancelled — {function_name} was skipped due to user interrupt]"
            )
            return True

        valid_tool_names = getattr(self, "valid_tool_names", None)
        if valid_tool_names is not None and function_name not in valid_tool_names:
            error = (
                f"Workflow action blocked: required tool '{function_name}' is not enabled "
                "for this session."
            )
            logger.warning(
                "Workflow action blocked by enabled-tool restrictions: session=%s tool=%s",
                getattr(self, "session_id", ""),
                function_name,
            )
            _append_synthetic_tool_result(json.dumps({"error": error}, ensure_ascii=False))
            return True

        if getattr(self, "iteration_budget", None) is None:
            self.iteration_budget = IterationBudget(getattr(self, "max_iterations", 0) or 0)
        if not self.iteration_budget.consume():
            error = (
                f"Workflow action blocked: iteration budget exhausted "
                f"({self.iteration_budget.used}/{self.iteration_budget.max_total} iterations used)."
            )
            logger.warning(
                "Workflow action blocked by iteration budget: session=%s used=%s max=%s",
                getattr(self, "session_id", ""),
    ...
    def _run_review_response_workflow_runtime_step(
        self,
        messages: list[dict[str, Any]],
        effective_task_id: str,
        original_user_message: str,
    ) -> Optional[str]:
        """Run deterministic review-response workflow actions before LLM narration.

        This is the production integration point for the Task 7 runner.  It is
        intentionally narrow to the targeted workflow and executes any tool work
        through ``execute_review_response_workflow_action()``, which in turn uses
        the standard tool-call path.
        """
        ...
        self._consume_review_response_final_approval(original_user_message)
        inputs = self._build_review_response_runner_inputs(original_user_message)
        for _ in range(4):
            action = self.next_review_response_workflow_action(inputs)
            if action is None:
                return None
            action_type = str(getattr(action, "action_type", "") or "")

            if getattr(action, "report", ""):
                final = str(action.report)
                messages.append({"role": "assistant", "content": final})
                return final

            terminal_args = getattr(action, "terminal_args", None)
            if terminal_args:
                self.execute_review_response_workflow_action(action, messages, effective_task_id)
                parsed = self._parse_latest_review_response_guard_result(messages)
                if parsed and action_type.endswith("duplicate_session_check"):
                    duplicate_active = bool(parsed.get("duplicate_session_active", False))
                    inputs = self._build_review_response_runner_inputs(
                        original_user_message,
                        duplicate_session_active=duplicate_active,
                    )
                    continue
                if parsed and action_type.endswith("actionable_state_check"):
                    runner_module = self._review_response_runner_module()
                    actionable = runner_module.ActionableStateCheck(
                        unresolved_review_threads=int(parsed.get("unresolved_review_threads") or 0),
                        failing_ci=bool(parsed.get("failing_ci", False)),
                        other_actionable_state=bool(parsed.get("other_actionable_state", False)),
                        ci_status=str(parsed.get("ci_status") or "not reported"),
                        local_verification=str(parsed.get("local_verification") or "not reported"),
                    )
                    inputs = self._build_review_response_runner_inputs(
                        original_user_message,
                ...
                finalization = self._parse_latest_review_response_marker_result(
                    messages,
                    "WORKFLOW_FINALIZATION_RESULT",
                )
                if finalization and action_type.endswith("finalize"):
                    if self.workflow_state is not None:
                        self.workflow_state.structured_results["finalization"] = dict(finalization)
                        self.workflow_state.finalization.finalized = bool(finalization.get("finalized", False))
                        self._persist_workflow_state()
                    inputs = self._build_review_response_runner_inputs(
                        original_user_message,
                        final_report_approved=True,
                        finalization_succeeded=bool(finalization.get("finalized", False)),
                    )
                    continue

                thread_resolution = self._parse_latest_review_response_marker_result(
                    messages,
                    "WORKFLOW_THREAD_RESOLUTION_RESULT",
                )
                if thread_resolution and action_type.endswith("resolve_threads"):
                    if self.workflow_state is not None:
                        self.workflow_state.structured_results["thread_resolution"] = dict(thread_resolution)
                        self._persist_workflow_state()
                    inputs = self._build_review_response_runner_inputs(
                        original_user_message,
                        threads_resolved=bool(thread_resolution.get("threads_resolved", False)),
                    )
                    continue

                if bool(terminal_args.get("background")):
                    final = str(getattr(action, "reason", "Workflow phase started."))
                    messages.append({"role": "assistant", "content": final})
                    return final
                final = str(getattr(action, "reason", "Workflow guard executed."))
                messages.append({"role": "assistant", "content": final})
                return final

            if action_type.endswith("wait") or "WAIT" in action_type:
                final = str(getattr(action, "reason", "Workflow is waiting for deterministic state."))
                messages.append({"role": "assistant", "content": final})
                return final
            if action_type.endswith("complete") or action_type.endswith("abort") or action_type.endswith("finalization_locked"):
                final = str(getattr(action, "reason", "Workflow state reached a deterministic boundary."))
                messages.append({"role": "assistant", "content": final})
                return final
            return None
        return None

    def _review_response_runner_module(self) -> Any:
        from importlib import import_module

        return import_module("agent.workflows.review_response_phase_runner")

    def _consume_review_response_final_approval(self, user_message: str) -> bool:
        """Persist an explicit final-approval user signal for the active workflow."""

        state = getattr(self, "workflow_state", None)
        if state is None:
            return False
        if state.phase != "final_approval_gate":
            return False
        if not getattr(state.approvals, "ready_for_final_report", False):
            return False
        if getattr(state.approvals, "final_report_approved", False):
            return False
        if not self._is_explicit_review_response_final_approval(user_message):
            return False
        ...
        state.loop_gates.terminal_reason = "Explicit user approval text received at the final approval gate."
        self._persist_workflow_state()
        return True

    @staticmethod
    def _is_explicit_review_response_final_approval(user_message: str) -> bool:
        text = re.sub(r"\s+", " ", str(user_message or "").strip().lower())
        if not text:
            return False
        approval_patterns = (
            r"\bapprove\s+(?:the\s+)?(?:final\s+)?(?:report|response|finalization)\b",
            r"\b(?:final\s+)?(?:report|response|finalization)\s+approved\b",
            r"\bapproved\s+for\s+(?:finalization|thread\s+resolution|final\s+report)\b",
            r"\bfinal\s+approval\s+(?:granted|approved)\b",
            r"\byes,?\s+(?:approve|approved)\b.*\b(?:final|report|finalization)\b",
        )
        return any(re.search(pattern, text) for pattern in approval_patterns)

    def _build_review_response_runner_inputs(self, user_message: str, **overrides: Any) -> Any:
        runner_module = self._review_response_runner_module()
        pr_url, pr_number = self._extract_review_response_pr(user_message)
        repo_path = os.getcwd()
        structured_overrides = self._structured_review_response_runner_overrides(runner_module)
        structured_overrides.update(overrides)
        return runner_module.ReviewResponseRunnerInputs(
            pr_url=pr_url,
            repo_path=repo_path,
            pr_number=pr_number,
    ...
    def _structured_review_response_runner_overrides(self, runner_module: Any) -> dict[str, Any]:
        """Rehydrate persisted structured phase outputs into runner inputs."""

        state = getattr(self, "workflow_state", None)
        if state is None:
            return {}
        structured_results = getattr(state, "structured_results", None)
        if not isinstance(structured_results, dict):
            return {}
        overrides: dict[str, Any] = {}
        pr_payload = structured_results.get("pr_analysis")
        if isinstance(pr_payload, dict):
            overrides["pr_analysis"] = runner_module.PRAnalysisResult(
                completed=bool(pr_payload.get("completed", False)),
                unresolved_review_threads=self._optional_int(pr_payload.get("unresolved_review_threads")),
                failing_ci=self._optional_bool(pr_payload.get("failing_ci")),
                modifications_made=self._optional_bool(pr_payload.get("modifications_made")),
                working_tree_clean=self._optional_bool(pr_payload.get("working_tree_clean")),
                root_cause_no_valid_issues=self._optional_bool(pr_payload.get("root_cause_no_valid_issues")),
                ci_status=str(pr_payload.get("ci_status") or "not reported"),
                local_verification=str(pr_payload.get("local_verification") or "not reported"),
            )
        changed_payload = structured_results.get("changed_review")
        if isinstance(changed_payload, dict):
            overrides["changed_review"] = runner_module.ChangedReviewResult(
                completed=bool(changed_payload.get("completed", False)),
                succeeded=bool(changed_payload.get("succeeded", False)),
                blocking_findings=tuple(self._string_list(changed_payload.get("blocking_findings"))),
                non_blocking_watch_items=tuple(self._string_list(changed_payload.get("non_blocking_watch_items"))),
                made_changes=bool(changed_payload.get("made_changes", False)),
                requires_human_confirmation=bool(changed_payload.get("requires_human_confirmation", False)),
                clean_recommendation=bool(changed_payload.get("clean_recommendation", False)),
                review_findings=str(changed_payload.get("review_findings") or ""),
            )
        return overrides
    ...
    def _optional_bool(value: Any) -> Optional[bool]:
        ...
        if text in {"true", "yes", "1", "success", "succeeded", "ok", "passed", "clean"}:
            return True
        if text in {"false", "no", "0", "failed", "failure", "error", "blocked", "dirty"}:
            return False
        return None
    ...
    def _extract_review_response_pr(self, user_message: str) -> tuple[str, int]:
        text = str(user_message or "")
        match = re.search(r"https://github\.com/[^\s)>'\"]+/[^\s)>'\"]+/pull/(\d+)", text)
        if match:
            return match.group(0), int(match.group(1))
        number_match = re.search(r"(?:PR|pull request)\s*#?(\d+)", text, flags=re.IGNORECASE)
        return "", int(number_match.group(1)) if number_match else 0

    def _parse_latest_review_response_guard_result(self, messages: list[dict[str, Any]]) -> Optional[dict[str, Any]]:
        return self._parse_latest_review_response_marker_result(messages, "WORKFLOW_GUARD_RESULT")

    def _parse_latest_review_response_marker_result(
        self,
        messages: list[dict[str, Any]],
        marker_name: str,
    ) -> Optional[dict[str, Any]]:
        for msg in reversed(messages):
            ...
            try:
                ...
                if isinstance(payload, dict):
                    output = str(payload.get("output") or payload.get("stdout") or content)
                else:
                    output = str(content)
            except Exception:
                output = str(content)
            marker = re.search(rf"{re.escape(marker_name)}=(\{{.*\}})", output, flags=re.DOTALL)
            if not marker:
                continue
            try:
    ...
    def _apply_workflow_tool_policy(
        self,
        function_name: str,
        function_args: dict[str, Any],
    ) -> tuple[dict[str, Any], Optional[str]]:
        """Apply targeted workflow delegate/terminal argument policy.

        This runs before plugin hooks and real dispatch so ordinary approvals,
        logging, checkpoints, and tool permissions continue to operate on the
        final argument shape.  Non-targeted workflows and unrelated tools are
        returned unchanged.
        """

        result = enforce_workflow_tool_policy(
            workflow_id=self._active_workflow_id(),
        ...
        if result.rewrites:
            logger.info(
                "Workflow policy rewrote %s args for session %s: %s",
                function_name,
                getattr(self, "session_id", ""),
                result.rewrites,
            )
        if result.blocked:
            self._persist_workflow_state()
        return result.args, result.error

    def _apply_workflow_tool_policy_compat(
        self,
        function_name: str,
        function_args: dict[str, Any],
    ) -> tuple[dict[str, Any], Optional[str]]:
        """Apply workflow policy when available, otherwise preserve stub compatibility."""

        apply_policy = getattr(self, "_apply_workflow_tool_policy", None)
        if callable(apply_policy):
            return cast(
                tuple[dict[str, Any], Optional[str]],
                apply_policy(function_name, function_args),
            )
        return function_args, None

    def _ingest_workflow_background_completion_message(self, message: str) -> None:
        """Consume existing background-completion notification text into workflow state."""

        completion = extract_background_completion_from_message(message)
        if not completion:
            return

        if not self._active_workflow_id():
            store = getattr(self, "_workflow_state_store", None)
            if store is None:
                store = ReviewResponseWorkflowStateStore()
                self._workflow_state_store = store
            session_id = getattr(self, "session_id", "") or "unknown"
            try:
                if not store.path_for_session(session_id).exists():
                    return
            ...
            self.workflow_context = {
                ...
                "skill_name": state.workflow_id,
                "policy_profile": state.workflow_id,
            }
            self.workflow_state = state

        if self.workflow_state is None:
            return
        if ingest_workflow_background_completion(self.workflow_state, **completion):
            self._persist_workflow_state()

    def _record_workflow_tool_result(
        self,
        function_name: str,
        function_args: dict,
        function_result: str,
        ...
        """Record workflow-owned background/delegation handles from normal tool results."""
        ...
        workflow_phase = str(function_args.get("workflow_phase") or self.workflow_state.phase or "")
        delegation_type = str(function_args.get("delegation_type") or "")
        changed = False
        if function_name == "terminal" and payload.get("session_id"):
            changed = record_workflow_background_handle(
                self.workflow_state,
                handle_id=str(payload.get("session_id") or ""),
                kind="terminal",
                ...
                metadata={
                    ...
                    "notify_on_complete": function_args.get("notify_on_complete"),
                },
            ) or changed
        elif function_name == "delegate_task" and isinstance(payload.get("results"), list):
            for entry in payload.get("results") or []:
                ...
                handle_id = str(
                    ...
                    or f"delegate:{getattr(self, 'session_id', '')}:{entry.get('task_index', '')}"
                )
                changed = record_workflow_background_handle(
                    self.workflow_state,
                    handle_id=handle_id,
                ...
                changed = ingest_workflow_background_completion(
                    self.workflow_state,
                    handle_id=handle_id,
                    exit_code=0 if entry.get("status") == "completed" else 1,
                    output=str(entry.get("summary") or entry.get("error") or ""),
                    command="delegate_task",
                    source="delegate_task_result",
                ) or changed
        if changed:
            self._persist_workflow_state()
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        return _delegate_task(
            ...
            notify_on_complete=function_args.get("notify_on_complete"),
            prompt_file=function_args.get("prompt_file"),
            prompt_file_path=function_args.get("prompt_file_path"),
            delivery_method=function_args.get("delivery_method"),
            workflow_phase=function_args.get("workflow_phase"),
            delegation_type=function_args.get("delegation_type"),
            parent_agent=self,
        )
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        if not pre_tool_block_checked:
            function_args, policy_error = AIAgent._apply_workflow_tool_policy_compat(self,
                function_name,
                function_args,
            )
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        for tool_call in tool_calls:
            ...
            function_args, workflow_policy_error = AIAgent._apply_workflow_tool_policy_compat(self,
                function_name,
                function_args,
            )

            # Checkpoint for file-mutating tools
            if workflow_policy_error is None and function_name in ("write_file", "patch") and self._checkpoint_mgr.enabled:
                try:
            ...
            if workflow_policy_error is None and function_name == "terminal" and self._checkpoint_mgr.enabled:
                try:
            ...
            if workflow_policy_error is not None:
                block_message = workflow_policy_error
            else:
                try:
                    from hermes_cli.plugins import get_pre_tool_call_block_message
                    block_message = get_pre_tool_call_block_message(
        ...
        def _run_tool(index, tool_call, function_name, function_args):
            ...
            try:
                ...
                invoke_target = getattr(self._invoke_tool, "side_effect", None) or self._invoke_tool
                try:
                    invoke_signature = inspect.signature(invoke_target)
                    invoke_params = invoke_signature.parameters
                    accepts_kwargs = any(
                        param.kind == inspect.Parameter.VAR_KEYWORD
                        for param in invoke_params.values()
                    )
                    if not accepts_kwargs:
                        invoke_kwargs = {
                            key: value
                            for key, value in invoke_kwargs.items()
                            if key in invoke_params
                        }
                except (TypeError, ValueError):
                    pass
        ...
        for i, (tc, name, args, block_result, blocked_by_guardrail) in enumerate(parsed_calls):
            ...
            if r is None:
                ...
                append_guardrail_observation = getattr(self, "_append_guardrail_observation", None)
                if not blocked and callable(append_guardrail_observation):
                ...
                record_workflow_tool_result = getattr(self, "_record_workflow_tool_result", None)
                if not blocked and callable(record_workflow_tool_result):
    ...
    def _execute_tool_calls_sequential(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        for i, tool_call in enumerate(assistant_message.tool_calls, 1):
            ...
            function_args, _workflow_policy_error = AIAgent._apply_workflow_tool_policy_compat(self,
                function_name,
                function_args,
            )

            # Check plugin hooks for a block directive before executing.
            _block_msg: Optional[str] = _workflow_policy_error
            ...
            if not _execution_blocked:
                self._record_workflow_tool_result(
                    function_name,
                    function_args,
                    function_result,
                    effective_task_id,
    ...
    def run_conversation(
        ...
        workflow_runtime_response = self._run_review_response_workflow_runtime_step(
            messages,
            effective_task_id,
            original_user_message if isinstance(original_user_message, str) else "",
        )
        workflow_runtime_active = workflow_runtime_response is None
        if workflow_runtime_response is not None:
            final_response = workflow_runtime_response
            _turn_exit_reason = "review_response_workflow_runtime"

        while workflow_runtime_active and ((api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call):
