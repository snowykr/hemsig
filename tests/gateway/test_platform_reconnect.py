...
class TestStartupPlatformIsolation:
    ...
    async def test_start_continues_after_platform_connect_timeout(self, tmp_path):
        """A timeout on Telegram should queue it and still connect later adapters."""
        runner = _make_runner()
        runner.config = GatewayConfig(
            platforms={
                Platform.TELEGRAM: PlatformConfig(enabled=True, token="test"),
                Platform.SIGNAL: PlatformConfig(enabled=True, token="test"),
            },
        ...
        adapters = {
            ...
            Platform.SIGNAL: StubAdapter(platform=Platform.SIGNAL),
        }
        ...
        assert Platform.SIGNAL in runner.adapters
