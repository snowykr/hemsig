...
class _CapturingAgent:
    ...
    def run_conversation(self, user_message, conversation_history=None, task_id=None, persist_user_message=None):
        if type(self).next_result is not None:
...
@pytest.mark.asyncio
async def test_run_agent_preserves_pdir_changed_to_from_agent_result(monkeypatch, tmp_path):
    _install_fake_agent(monkeypatch)
    runner = _make_runner()
    ...
    _CapturingAgent.next_result = {
        "final_response": "Project directory changed",
        "messages": [],
        "api_calls": 0,
        "completed": True,
        "cwd_changed_to": target,
        "pdir_changed_to": target,
    }

    result = await runner._run_agent(
        message=f"pdir ->> {target}",
        context_prompt="Context prompt",
        history=[],
        source=_make_source(),
        session_id="session-1",
    ...
    assert result["cwd_changed_to"] == target
    assert result["pdir_changed_to"] == target
